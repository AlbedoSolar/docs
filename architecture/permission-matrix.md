...
| `user_codes` | sales, admin | sales, admin | — | SELECT public; writes gated by `is_sales_or_admin()` (2026-09-08). DELETE intentionally not granted (FKs from `estimates`, `estimate_vendors`, `employees`) — expire a code via `active_until`. The `/user-codes` delete button therefore matches 0 rows for non-admins. |
