...
6. ~~**Manual mode is still a second engine**~~ *(FIXED 2026-07-13, ruling: keep manual mode, align the calc. `runManualMode` now maps its raw inputs onto the same shared pipeline (`runForwardCalc` in quote-forward-calc) — Subtract margins grossed up like everywhere else. The parallel R-port engine (`generateNiiTable`/`getKpis`/`amortForm`/`amortTimeForm`, ~460 lines) is DELETED from quote-calculator.ts. Forensics: manual mode had only ever run 56 times, all dev tests (Jake/Ian/Danniel), last 2026-02-21. Equivalence proven: manual vs estimate mode on identical inputs → all 10 KPIs and all 61×~40 cashflow cells byte-identical. Deploy tag `deploy/quote-generator/20260713T173900Z-d02b743`.)*
...
10. **Engine-internal clones**: ~~frontend-shape mappers redefined verbatim in quote-generator~~ *(FIXED 2026-07-13 — imports the canonical quote-forward-calc exports)*; ~~getWithFallback copied~~ *(FIXED 2026-07-10 — hoisted to quote-params.ts)*; ~~IRR bisection stack duplicated in quote-calculator~~ *(GONE 2026-07-13 — deleted with the manual-mode engine; quote-integration-flow's copy is now the only one)*; `round2` ×2 remains (quote-integration-flow + quote-calculator).
